...
export function ExamCard({ exercise, index }: ExamCardProps) {
  const [answers, setAnswers] = useState<Record<string, string>>({});
  const [showSolution, setShowSolution] = useState(false);

  const categoryLabel =
    exercise.category === "upper_body" ? "גוף עליון" : "גוף תחתון";
  const categoryColor =
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-700/60 rounded-2xl overflow-hidden shadow-xl">
      <div className="px-6 py-5 border-b border-zinc-700/60 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <span className="text-zinc-500 font-mono text-sm">#{index + 1}</span>
          <h2 className="text-white text-xl font-bold tracking-tight" dir="rtl">
            {exercise.name}
          </h2>
        </div>
          ...
          {showSolution && (
            <div className="mt-4 rounded-xl border border-amber-700/40 bg-amber-950/20 overflow-hidden">
              <div className="px-4 py-3 border-b border-amber-700/40 bg-amber-900/20">
                <p className="text-amber-400 font-semibold text-sm" dir="rtl">
                  ✓ פתרון מוצע
                </p>
              </div>
